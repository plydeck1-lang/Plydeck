...
export async function GET(request:Request){try{
 ...
 let profile=null,orders:any[]=[],whatsappMessages:any[]=[];
 if(user){const profileResult=await db.from('profiles').select('*').eq('id',user.id).maybeSingle();profile=profileResult.data;let oq=db.from('orders').select('*').order('created_at',{ascending:false});if(!isAdmin)oq=oq.eq('user_id',user.id);const or=await oq;if(or.error)throw or.error;orders=or.data??[];let mq=db.from('whatsapp_messages').select('*').order('created_at',{ascending:false}).limit(200);if(!isAdmin)mq=mq.eq('user_id',user.id);const mr=await mq;if(mr.error)throw mr.error;whatsappMessages=mr.data??[];}
 ...
 return ok({categories:categories.filter(c=>isAdmin||c.active),pools,shipments,profile,orders,isAdmin,waitlist:waiting.filter(w=>isAdmin||w.user_id===user?.id),whatsappMessages,whatsapp});
