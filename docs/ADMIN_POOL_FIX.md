...
- Creating a pool automatically uses the current internal delivery batch. The pool editor no longer asks the admin to choose a shipment, upload a pool image or enter operations allocations.
- Public pool listings use responsive, image-free cards. A saved draft stays private. Publishing it makes it appear automatically in its selected visible category and city.
...
4. Review the locked four-item quantities, enter all four rate-per-sft values and the product specification, tick the acknowledgement and save.
5. The saved pool remains a private draft. Click **Publish pool** to make it visible automatically in the selected category and city. A visible category and sufficient internal delivery-batch capacity are still required.
...
For a separate truckload, create that delivery batch first under **Manage → Shipments → Add shipment** and enter the carrier-confirmed legal cargo payload and packing allowance. Pool creation uses the current batch automatically. The database performs the final payload check before publication.

The patch does not publish pools, alter product specifications, take payments or send WhatsApp messages automatically.

## Verification

- All 23 automated tests passed, including date-offset regression tests and local PostgreSQL-compatible tests for draft save, publication guards and creating a pool on another shipment.
