export type PoolStatus = 'draft'|'live'|'confirming'|'confirmed'|'qc_ready'|'dispatched'|'cancelled';
...
export type Category={id:string;name:string;slug:string;description:string;image_url:string;active:boolean};
export type Shipment={id:string;name:string;origin:string;destination:string;payload_kg:number;packing_kg:number};
export type Pool={id:string;code:string;name:string;category_id:string;shipment_id:string;city:string;image_url:string;status:PoolStatus;total_slots:number;closes_at:string;config:PoolConfig;updated_at?:string;confirmed_at?:string;delivery_target?:string;qc_report?:string;payment_due_at?:string;allocations?:{slot_no:number;status:string}[];waitlist_count?:number};
export type Profile={id?:string;business_name:string;contact_name:string;phone:string;gstin:string;address:string;city:string;pincode:string;state:string;email?:string;whatsapp_opt_in?:boolean};
export type QuoteLine={label:string;amount:number;basis:string};
export type Quote={primary_qty:number;secondary_qty:number;slot_count:number;sheets:number;area:number;weight_kg:number;lines:QuoteLine[];subtotal:number;gst:number;total:number;rate:number;stages:Record<Stage,number>;terms_version:string};
